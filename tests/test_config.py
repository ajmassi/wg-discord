...
base_env = {
    ...
    "guild_private_key": b64encode(bytes(sha512(b"GUILD_PRIVATE_KEY").hexdigest()[:32].encode())),
    "guild_public_key": b64encode(bytes(sha512(b"GUILD_PUBLIC_KEY").hexdigest()[:32].encode())),
...
def test_false_guild_save_config(value):
    test_env = base_env
    test_env.update({
        "guild_save_config": value
    })
    print(test_env)
    config = WireGuardSettings(**test_env)
    assert config.guild_save_config == False
...
def test_true_guild_save_config(value):
    ...
    print(test_env)
    config = WireGuardSettings(**test_env)
    assert config.guild_save_config == True