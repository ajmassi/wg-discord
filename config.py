...
class WireGuardSettings(BaseSettings):
    ...
    class Config:
        @classmethod
        def cast_ip(cls, raw_ip: str) -> ipaddress.IPv4Network | ipaddress.IPv6Network | None:
            try:
                return ipaddress.ip_network(raw_ip, strict=False)
            except ValueError:
                log.error(f"Unable to cast {raw_ip} as IPv6, verify the value is a valid network address.")
            return None

        @classmethod
        def parse_env_var(cls, field_name: str, raw_val: str) -> Any:
            if field_name in ('guild_interface_reserved_network_addresses', 'guild_interface_dns', 'user_allowed_ips'):
                # Cast each list member and create list of valid IPs
                return [ip for raw_ip in raw_val.split(',') if (ip := cls.cast_ip(raw_ip)) is not None]
